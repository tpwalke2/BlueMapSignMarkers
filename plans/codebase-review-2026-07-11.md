...
**Resolved 2026-07-23.** `SignLinesParser`'s constructor now test-compiles every `REGEX` group's prefix with
`Pattern.compile(...)` once, up front, logging one warning per invalid group and dropping it from the list used
for matching — instead of catching `PatternSyntaxException` on every sign parsed. A malformed REGEX prefix on
one group no longer blocks matching against the groups after it or crashes sign processing, and the log only
gets one line per bad group instead of one per sign.
