...
- `allowPlayerColors` - lets a player set the marker's colour by dyeing one of its member signs (right-clicking it with a dye item), instead of an admin editing `lineColor`/`fillColor`; among the marker's current members, the earliest-placed member with a non-dyed (non-black) colour wins, re-evaluated on every membership/dye change; only the hue comes from the dye, the alpha is still taken from `lineColor`/`fillColor`; optional; default is `false`; `LINE`/`SHAPE`/`EXTRUDE` only. **Note:** an undyed sign and a sign dyed black look identical to Minecraft, so a black dye can never win - dyeing a member black has no visible effect and the group falls back to `lineColor`/`fillColor` as if nothing were dyed.
