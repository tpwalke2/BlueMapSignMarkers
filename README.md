...
- `allowPlayerColors` - lets a player set the marker's colour by dyeing one of its member signs (right-clicking it with a dye, ink sac, or glow ink sac item), instead of an admin editing `lineColor`/`fillColor`; among the marker's current members, the earliest-placed member with a non-dyed (non-black) colour wins, re-evaluated on every membership/dye change; only the hue comes from the dye, the alpha is still taken from `lineColor`/`fillColor`; optional; default is `false`; `LINE`/`SHAPE`/`EXTRUDE` only. **Note:** an undyed sign and a sign dyed black look identical to Minecraft, so a black dye can never win - dyeing a member black has no visible effect and the group falls back to `lineColor`/`fillColor` as if nothing were dyed.

Setting a field on a group type it doesn't apply to (e.g. `icon` on a `LINE`/`SHAPE`/`EXTRUDE` group, `fillColor` on a
`POI`/`LINE` group, `depthTest` on a `POI` group, `allowPlayerColors` on a `POI` group, or `cssClasses` on a
`LINE`/`SHAPE`/`EXTRUDE` group) is not an error; the mod logs a warning and ignores the field.
