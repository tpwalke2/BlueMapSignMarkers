...
- `allowPlayerColors` - lets a player set the marker's colour by dyeing one of its member signs (right-clicking it with a dye, ink sac, or glow ink sac item), instead of an admin editing `lineColor`/`fillColor`; among the marker's current members, the earliest-placed member with a non-dyed (non-black) colour wins, re-evaluated on every membership/dye change; only the hue comes from the dye, the alpha is still taken from `lineColor`/`fillColor`; optional; default is `false`; `LINE`/`SHAPE`/`EXTRUDE` only
