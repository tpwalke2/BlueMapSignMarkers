...
`01-render-mask-semantics-and-config-lookup.md`'s resolved
...
- Where the render-mask evaluator (parsing `config/bluemap/maps/<id>.conf` per the hand-rolled
  parser from `02-hocon-parsing-approach.md`) lives, and when
...
Grilled to five decisions (see `../map.md` for the one-line gists);
...
`01-render-mask-semantics-and-config-lookup.md`, using the
hand-rolled parser approach from
`02-hocon-parsing-approach.md`. Fails open (treats as
