...
| `common` | `SafeCall` — `run(label, Runnable)`, wraps a lifecycle-hook/mixin entry point in a catch-log-and-continue so a single bad event never crashes the server (see `AGENTS.md`'s "Mod must never crash the server" principle); `ServerPathResolver` — `resolveMarkerStorageRoot`/`resolveLegacyMarkerFilePath(Path rawLevelPath)`, pure `Path`/`String` math extracted from `BlueMapSignMarkersMod` (Minecraft-coupled) so it's directly unit-testable (see `config-and-persistence.md`); `FileUtils` — `createBackup`/`moveToBackup(originalPath, suffix, description)` (copy- vs. rename-based backup, used by config/sign migration code before overwriting or retiring an old-format file); `HtmlUtils` — `escape`/`toHtmlDetail`, escapes player-supplied sign text before it crosses into BlueMap's HTML-rendered marker `detail` field (`escape(null)` returns `""`); `ColorUtils` — `parseHex(String) -> int[]{r,g,b,a}`, parses a line marker's hex color (`"#RRGGBB"`/`"#RRGGBBAA"`, leading `#` optional), falling back to opaque red with a log rather than throwing on malformed input |
...
| `core.bluemap.actions` | `MarkerAction` (abstract base, holds a `DispatchedMarkerIdentifier`), `AddMarkerAction`/`UpdateMarkerAction`/`RemoveMarkerAction`, `SetMultiPointMarkerAction` (create/update a line/shape/extrude's rendered points — one class for all three kinds, carrying `lineWidth`/`lineColor`/optional `fillColor`, the full current point list, and a log-only `isFirstAppearance` flag; replaced the former separate `SetLineMarkerAction`/`SetShapeMarkerAction`/`SetExtrudeMarkerAction`, which were structurally identical apart from `fillColor` presence and their minimum-points threshold), `RemoveMultiPointMarkerAction` (a line/shape/extrude drops back below its minimum member count; handles all three kinds via `MultiPointMarkerIdentifier`'s `kind` field — replaced the separate `RemoveLineMarkerAction`/`RemoveShapeMarkerAction`/`RemoveExtrudeMarkerAction`, which were structurally identical apart from their `toString()` class-name literals), `GroupTransitionMarkerAction` (bundles 0-2 `MarkerAction` effects so a sign's representation change — a `POI`/`LINE`/`SHAPE`/`EXTRUDE` type flip, or a prefix change between groups — dispatches leave+join atomically; replaced the deleted `ChangeGroupMarkerAction`), `ActionFactory` (builds actions, resolves `MarkerSetIdentifier`s — one factory-method pair, `createSetMultiPointAction`/`createRemoveMultiPointAction`, covers `LINE`/`SHAPE`/`EXTRUDE` alike, deriving kind/minimum-members from `MarkerGroup.type()`; `createGroupTransitionPOIAction` is the `POI`↔`POI` group-change counterpart, renamed from `createChangeGroupPOIAction`) |
| `core.markers` | `MarkerGroup` (record: the config unit; gained `lineWidth`/`lineColor`/`fillColor` trailing fields — `fillColor` is `SHAPE`/`EXTRUDE`-only), `MarkerGroupMatchType` (`STARTS_WITH`/`REGEX`), `MarkerGroupType` (`POI`, `LINE`, `SHAPE`, or `EXTRUDE`), `MarkerIdentifier`/`MarkerSetIdentifier`/`MarkerSetIdentifierCollection` (marker identity + dedup), `DispatchedMarkerIdentifier` (interface: `parentSet()`/`getId()`, generalizes over position-keyed and content-keyed marker ids), `MultiPointMarkerIdentifier` (record: `kind`, `label`, `parentSet`; `getId()` is `kind + ":" + label` — content-keyed, not position-keyed; `kind` is `"line"`/`"shape"`/`"extrude"` — replaced the separate `LineMarkerIdentifier`/`ShapeMarkerIdentifier`/`ExtrudeMarkerIdentifier` records, which differed only by that hardcoded id-prefix), `LinePoint` (record: `x, y, z`, one rendered point of a line/shape/extrude volume) |
